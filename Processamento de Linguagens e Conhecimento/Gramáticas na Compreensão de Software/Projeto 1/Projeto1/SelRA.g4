...
linhacaracteristica[File f, HashMap<String,ArrayList<String>> mapCaracteristicasIn]
    ...
    @init { $mapCaracteristicasOut = new HashMap<String,ArrayList<String>>(); } :
            ...
            LPAR sinonimos[$f] RPAR                 {   if(!($mapCaracteristicasIn.containsKey($caracteristicaprincipal.text))){
                                                            if(!($sinonimos.lista.contains($caracteristicaprincipal.text))){
                                                                $sinonimos.lista.add(($caracteristicaprincipal.text).toLowerCase());
                                                            }
                                                            $mapCaracteristicasIn.put(($caracteristicaprincipal.text).toLowerCase(),$sinonimos.lista);
                                                        }
                                                        else{
                                                            String s = "<p> ERRO: Característica " + $caracteristicaprincipal.text + " já referida... </p>\n" ;
                                                            escreveFicheiro($f,s);
                                                        }
...
recurso[File f, HashMap<String,Recurso> mapRecursosIn]
    ...
    @init { $mapRecursosOut = new HashMap<String,Recurso>(); } :
        idrecurso desc intervaloidade[$f,$idrecurso.text] LPAR c=carateristicas[$f] RPAR
...
aluno[File f, HashMap<String,Aluno> mapAlunosIn]
    ...
    @init {$mapAlunosOut = new HashMap<String,Aluno>();} :
        idaluno idade LPAR c=carateristicas[$f] RPAR
                                                    {   Aluno a = new Aluno();
                                                        a.id = $idaluno.text;
