...
/**
 * Write a description of class FBFeed.TesteFBFeed here.
 *
 * @author (your name)
...
public class TesteFBFeed
{
    public static void main(String[] args) {
        ...
        p = feed.getPost(3);
        if(p.getUser().equals("User 3")) {
            System.out.println("getPost ok");
        } else {
            System.out.println("Erro em getPost");
        }

        feed.comment(p.getId(), "Primeiro comentario");
        if(p.getComents().get(0).equals("Primeiro comentario")) {
            System.out.println("comment ok");
        } else {
            System.out.println("Erro em comment");
        }

        feed.comment(p.getId(), "Segundo comentario");
        if(p.getComents().get(1).equals("Segundo comentario")) {
            System.out.println("comment ok");
        } else {
            System.out.println("Erro em comment");
        }

        feed.comment(post2,"coment");
        feed.comment(post2,"coment");
        feed.comment(post2,"coment");
        feed.comment(post1,"coment");
        feed.comment(post1,"coment");
        feed.comment(post1,"coment");
        feed.comment(post1,"coment");
        ...
        List<Integer> l = feed.top5CommentsI();
        if(l.size()==5) {
